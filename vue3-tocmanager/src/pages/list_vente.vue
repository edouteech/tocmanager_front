...
<div class="contenu">
    ...
    <table class="tableau">
        ...
        <tr  v-for="(vente, i) in ventes" :key="i">
          ...
          <td>
            <router-link :to="'/modifiervente/'+vente.id"><button>Modifier</button></router-link>
...
export default {
  ...
  methods: {
    deleteVente(id){
      console.log(id);
       axios
      .delete('http://localhost:8000/api/delete/vente/' +id)
      .then(response => 
  ...
  mounted () {
    axios
    .get('http://localhost:8000/api/index/vente')
    .then(response => 
        {console.log(response);
