    ...
    <div class="app-main__outer py-5 px-2">
        ...
        <div class="row mx-auto" >
            <div class="col-md-4 mx-auto offre" v-for="(plan, i) in plans" :key="i">
                <h4 class="text-center">{{plan.name}}</h4><p class='text-center text-danger' v-if="plan.name == planActif.name">Est votre abonnement actif <i class="fa fa-info-circle text-primary cursor-pointer" aria-hidden="true" @click.prevent="infoAbonnement()" title="Voir les informations de mon abonnement"></i></p><hr>
                ...
                <div>
                    ...
                    v-if="plan.name == planActif.name">
                        Renouveler pour un montant de <span class="text-success">{{plan.price}} {{plan.currency}}</span>
                    </button>
                    <button class="btn btn-outline-primary btn-offer p-2 my-4" @click="createAbonnement(plan)"
                        id="pay-btn"
                    v-else>
                        Changer pour un montant de <span class="text-success">{{plan.price}} {{plan.currency}}</span>
                    </button>
                </div>
            </div>
...
<voirAbonnement :infoPlan= 'infoPlans' v-show="showModal" @close-modal="showModal = false"/>  
</div>
</template>
                    ...
                    onComplete: (result => {
                        ...
                        if (result.transaction.status == "approved" ) {
                            this.$toast('Paiement effectué avec succès !!!', {
                                ...
                                this.$axios.post('/compagnie/change_plan',{ 
                                    compagnie_id: localStorage.getItem('auth.company_id'),
                                    plan_id: plan.id,
                                    params: {
                                        compagnie_id: localStorage.getItem('auth.company_id')
                                ...
                                .then(response => 
                                {
                                    ...
                                    if(response.data.status == "success"){
                                        this.$router.push( '/dashboard',)
                                        this.$toast("Vous venez de changer votre type d'abonnement !!!", {
                                            icon: 'fa fa-check-circle',
                                        })
                                    }else{
                                        this.error = response.data.message
