<template >
  <div>
      ...
      <div class="app-main__outer py-5 px-2">
        <h4>Liste des bons de commande enregistrés</h4><hr><br>
        ...
        <div class="d-flex">
          ...
          <NuxtLink  to="/bons/add_bon" v-for="(user, i) in users" :key="i" class="web-btn"><button class="custom-btn btn-3" v-if="compagny == user.pivot.compagnie_id && user.pivot.droits_add == 1"><span>Ajouter un bon de commande</span></button></NuxtLink>
        </div>
        <div class="mobile-btn my-4">
          <NuxtLink  to="/bons/add_bon" v-for="(user, i) in users" :key="i"><button class="custom-btn btn-3" v-if="compagny == user.pivot.compagnie_id && user.pivot.droits_add == 1"><span>Ajouter un bon de commande</span></button></NuxtLink>
        </div>
        ...
        <div class="table-responsive">
          <table class="table table-hover" v-if="this.element_search == ''">
            ...
            <tbody>
             <tr  v-for="(bon, i) in bons" :key="i">
                  <!-- <td v-if="selection != 0"><div class="form-check"><input type="checkbox" v-model="checks" @change="checkbox(bon.id)" :value="bon.id"/></div></td> -->
                <td>{{bon.date_order}}</td>
                <td>{{bon.supplier.name}}</td>
                <td>{{bon.discount}}</td>
                <td>{{bon.amount}}</td>
                <td><div class="action d-flex aligns-items-center justify-content-center" v-for="(user, i) in users" :key="i">
                  <NuxtLink :to="'/bons/voir/'+bon.id" v-if=" compagny == user.pivot.compagnie_id"><i class="fa fa-info-circle text-info" aria-hidden="true"></i></NuxtLink>
                  <NuxtLink :to="'/bons/'+bon.id" v-if=" compagny == user.pivot.compagnie_id && user.pivot.droits_edition == 1"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></NuxtLink>
                  <div @click="deleteClient(bon.id)" v-if=" compagny == user.pivot.compagnie_id && user.pivot.droits_delete == 1"><i class="fa fa-trash-o text-danger" aria-hidden="true"></i></div>
                </div>
                </td>
              </tr>
            </tbody>
          </table>
          <p class="text-center"><strong>{{total}} bons au total </strong></p><hr class="text-primary">
        </div><br><br>
      ...
      methods: {
        ...
        submitFile(){
          ...
          refresh(page=1){
            ...
            .then(response => {
              // console.log(response);
            this.bons = response.data.data.data 
