<template>
  <div class="modal-overlay" @click="$emit('close-modal')">
    <div class="modaler text-center py-5">
      <strong>Voulez vous supprimer cette compagnie ???</strong><br /><br />
      <div class="d-flex" @click="$emit('close-modal')">
        <button class="btn btn-danger mx-auto" @click.prevent="sup()">
          Oui
        </button>
        <button class="btn btn-dark mx-auto">Non</button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    sup() {
      this.$axios
        .delete("/delete/compagnie/" + this.identifiant, {
          params: {
            compagnie_id: this.$auth.$storage.getUniversal("company_id"),
          },
        })
        .then((response) => this.$emit("conf", { message: this.identifiant }));
    },
  },
};
</script>
