<template>
  <div>
            ...
            <tr v-for="(result, i) in results" :key="i">
              ...
              <td>
                <div class="action d-flex aligns-items-center justify-content-center" v-for="(user, i) in users" :key="i">
                  <div @click="voirDecaissement(result.id)" v-if="compagny == user.pivot.compagnie_id">
                    <i class="fa fa-info-circle" aria-hidden="true"></i>
                  </div>
                  <NuxtLink :to="'/decaissements/' + result.id" v-if="
                    compagny == user.pivot.compagnie_id &&
                    user.pivot.droits_edition == 1
                  "><i class="fa fa-pencil-square-o" aria-hidden="true"></i></NuxtLink>
                  <div @click="deleteDecaissement(result.id)" v-if="
                    compagny == user.pivot.compagnie_id &&
                    user.pivot.droits_delete == 1
                  ">
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(decaissement, i) in decaissements" :key="i">
              <td v-if="selection != 0">
                <div class="form-check">
                  <input type="checkbox" v-model="checks" @change="checkbox(decaissement.id)" :value="decaissement.id" />
                </div>
              </td>
              ...
              <td>
                <div class="action d-flex aligns-items-center justify-content-center" v-for="(user, i) in users" :key="i">
                  <div @click="voirDecaissement(decaissement.id)" v-if="compagny == user.pivot.compagnie_id">
                    <i class="fa fa-info-circle" aria-hidden="true"></i>
                  </div>
                  <NuxtLink :to="'/decaissements/' + decaissement.id" v-if="
                    compagny == user.pivot.compagnie_id &&
                    user.pivot.droits_edition == 1
                  "><i class="fa fa-pencil-square-o" aria-hidden="true"></i></NuxtLink>
                  <div @click="deleteDecaissement(decaissement.id)" v-if="
                    compagny == user.pivot.compagnie_id &&
                    user.pivot.droits_delete == 1
                  ">
                    <i class="fa fa-trash-o text-danger" aria-hidden="true"></i>
    ...
    <addDecaissementModal v-show="addModal" @close-modal="addModal = false" @conf="setMessage" />
  </div>
</template>
...
import addDecaissementModal from './addDecaissementModal.vue'
export default {
  layout: "empty",
  auth: true,
  components: {
