    ...
    <div class="app-main__outer py-5 px-2">
      ...
      <div class="table-responsive">
        <table class="table table-hover">
          ...
          <tbody>
            <tr
              v-for="(decaissement, i) in decaissements"
              :key="i"
              class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
            >
              <td>{{ decaissement.date }}</td>
              <td>{{ decaissement.montant }}</td>
              <td v-if="decaissement.supplier">
                {{ decaissement.supplier.name }}
              </td>
...
export default {
  ...
  mounted() {
    //liste des décaissements supprimés
    this.$axios
      .get("/get/decaissements", {
        params: {
  ...
  methods: {
    //restaurer le décaissement
    restaurerDecaissement(id) {
      this.$axios
        .get("/restore/decaissement/" + id, {
          params: {
            compagnie_id: localStorage.getItem("auth.company_id"),
          },
        })
