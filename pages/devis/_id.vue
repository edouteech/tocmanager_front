...
export default {
  ...
  methods: {
    ...
    //rappel de la fonction recupProduct après le modal
    setProduit(payload) {
      this.recupProduct();
    },

    //recherche le produit par son code
    searchCode() {
      this.$axios
        .get("/products", {
          params: {
    ...
    async submit() {
      this.load = true;
      await this.$axios
        .put("/devis/" + this.$route.params.id, {
    ...
    //modifier la quantité dans les lignes de vente
    ...
    productChange(IdProduit, IndexSellLines) {
      ...
      for (let k = 0; k <= this.produits.length; k++) {
        if (this.produits[k].id == IdProduit) {
          ...
          this.form.devis_lines[IndexSellLines].amount_after_discount =
            ProdPrice;
          let sum = 0;
          for (let j = 0; j < this.form.devis_lines.length; j++) {
            sum += this.form.devis_lines[j].amount_after_discount;
