            ...
            <div class="alert alert-danger justify-content-center" role="alert" v-if="error">
                ...
                <form action="" method="POST">
                    ...
                    <div class="col-md-7 mx-auto table-responsive">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td><div  @change="taxChange()"><input class="form-control received" type="text" v-model="form.discount"  autocomplete="off"  required @change="reduceAmount()"></div></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td><input class="form-control received" type="number" v-model="form.amount_ttc"  autocomplete="off"  disabled></td>
                            </tr>
                        </tbody>
                    </table><br>
                    </div>
                    ...
                    <div class="col-md-6 mx-auto">
                        <button class="custom-btn btn-5 col-md-12" v-on:click.prevent="submit()" :disabled="load">Modifier le dévis</button>
                    </div>
                </form>
    ...
    export default {
        ...
        data () {
            return{
                ...
                form:{
                    user_id: '',
                    date_devis: moment().format("YYYY-MM-DDThh:mm"),
                    client_id: '',
                    amount: '',
                    tax: 0,
                    discount: 0,
        ...
        methods: {
            ...
            recupDevis(){
                this.$axios.get('/devis/'+ this.$route.params.id,{params: {
            ...
            addLine(){
                this.form.devis_lines.push({product_id: "", price: 0, quantity: 1, discount: 0, amount: 0, amount_after_discount: 0, compagnie_id: localStorage.getItem('auth.company_id'), date: this.form.date_sell});           
            },
            ...
            async codeAdd(){
              ...
              .then(response => {
                ...
                if(response.data.status == "success"){
                    ...
                    this.codeProd = "",
                    this.form.devis_lines.push({product_id: codeProdId, price: codeProdPrice, quantity: 1, discount: 0, amount: codeProdPrice*1, amount_after_discount: codeProdPrice*1, compagnie_id: localStorage.getItem('auth.company_id'), date: this.form.date_sell});  
                    this.form.discount = 0;  
                    this.form.tax = 0;  
                        let sum = 0;
                        for (let j = 0; j < this.form.devis_lines.length; j++) {
                            sum += this.form.devis_lines[j].amount_after_discount;
                        }
            ...
            deleteLine(index){
                //   console.log(this.form.sell_lines[index].id);
                    this.form.discount = 0
                    this.form.tax = 0
                if(this.form.devis_lines[index].id){
                    this.form.devis_lines[index]["_destroy"] = 1
                    let lineDestroy = this.form.devis_lines[index].amount_after_discount
                    this.$forceUpdate()
                    let sum = 0;
                    for (let j = 0; j < this.form.devis_lines.length; j++) {
                        sum += this.form.devis_lines[j].amount_after_discount;
                    ...
                    this.form.devis_lines.splice(index, 1)
                    let sum = 0;
                    for (let j = 0; j < this.form.devis_lines.length; j++) {
                        sum += this.form.devis_lines[j].amount_after_discount;
                    }
            ...
            async submit(){
                ...
                this.load = true
                await this.$axios.put('/devis/' +this.$route.params.id,{
                    id: this.$route.params.id,
                    date_devis: this.form.date_devis,
                    tax: this.form.tax,
                    ...
                    devis_lines: this.form.devis_lines,
                    compagnie_id: localStorage.getItem('auth.company_id')
                }).then(response =>{ 
                    ...
                    if(response.data.status == 'success'){
                        ...
                        this.$router.push({path:'/devis/list_devis'})
                    }
                    else{
                        this.load = false
                        this.error = response.data.message
            ...
            quantityChange(index, prod){
                ...
                let line = this.form.devis_lines[index]
            ...
            productChange(IdProduit, IndexSellLines){
                ...
                for(let k = 0; k <= this.produits.length; k++){
                    if(this.produits[k].id == IdProduit){
                        ...
                        // this.form.devis_lines.push({product_id: ProdId, price: ProdPrice, quantity: 1, discount: 0, amount: ProdPrice, amount_after_discount: ProdPrice, compagnie_id: localStorage.getItem('auth.company_id'), date: this.form.date_sell});  
                        this.form.devis_lines[IndexSellLines].product_id = ProdId
                        this.form.devis_lines[IndexSellLines].price = ProdPrice
                        this.form.devis_lines[IndexSellLines].quantity = 1
                        this.form.devis_lines[IndexSellLines].discount = 0
                        this.form.devis_lines[IndexSellLines].amount = ProdPrice
                        this.form.devis_lines[IndexSellLines].amount_after_discount = ProdPrice
                        // this.form.devis_lines.splice(this.form.devis_lines.length - 2, 1);  
                        let sum = 0;
                        for (let j = 0; j < this.form.devis_lines.length; j++) {
                            sum += this.form.devis_lines[j].amount_after_discount;
                        }
