...
export default {
  ...
  mounted() {
    //liste des encaissements supprimés
    this.$axios
      .get("/get/encaissements", {
        params: {
  ...
  methods: {
    //restaurer un encaissement
    restaurerEncaissement(id) {
      this.$axios
        .get("/restore/encaissement/" + id, {
          params: {
            compagnie_id: localStorage.getItem("auth.company_id"),
          },
        })
        .then((response) => {
          if (response.data.status == "success") {
            ...
            this.$toast("Restauration de l'encaissement !!!", {
    ...
    //modal de suppression de l'encaissement
    supEncaissement(id) {
      this.showModal = true;
      this.identifiant = id;
    },
