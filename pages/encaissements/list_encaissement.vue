  ...
  <div>
    ...
    <div class="app-main__outer p-5">
      ...
      <div class="d-flex">
        ...
        <NuxtLink to="/encaissements/encaissement" v-for="(user, i) in users" :key="i" class="web-btn"><button class="custom-btn btn-3"
            v-if="compagny == user.pivot.compagnie_id && user.pivot.droits_add == 1"><span>Remplir
              encaissement</span></button></NuxtLink>
      </div>

      <div class="mobile-btn my-4"><NuxtLink to="/encaissements/encaissement" v-for="(user, i) in users" :key="i"><button class="custom-btn btn-3"
            v-if="compagny == user.pivot.compagnie_id && user.pivot.droits_add == 1"><span>Remplir
              encaissement</span></button></NuxtLink>
      ...
      <div v-if="this.element_search != ''" class="table-responsive">
            ...
            <tr v-for="(result, i) in results" :key="i">
              ...
              <td>
                <div class="action" v-for="(user, i) in users" :key="i">
                  ...
                  <div @click="deleteEncaissement(result.id)"
        ...
        <p class="text-center"><strong>{{ total }} encaissements au total </strong></p>
              ...
              <td>
                <div class="action" v-for="(user, i) in users" :key="i">
                  ...
                  <NuxtLink :to="'/encaissements/' + encaissement.id"
                    v-if="compagny == user.pivot.compagnie_id && user.pivot.droits_edition == 1"><i
                      class="fa fa-pencil-square-o" aria-hidden="true"></i></NuxtLink>
                  <div @click="deleteEncaissement(encaissement.id)"
                    v-if="compagny == user.pivot.compagnie_id && user.pivot.droits_delete == 1"><i
                      class="fa fa-trash-o text-danger" aria-hidden="true"></i></div>
                  <NuxtLink :to="'/ventes/voir/' + encaissement.sell_id" v-b-tooltip.hover title="Accéder à la facture"
                    v-if="compagny == user.pivot.compagnie_id && encaissement.sell_id"><i
                      class="fa fa-eye text-success" aria-hidden="true"></i></NuxtLink>
                </div>
              </td>
            </tr>
          </tbody>
