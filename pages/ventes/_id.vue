<template>
  <div>
    ...
    <div class="app-main__outer py-5 px-2">
      ...
      <h4>Modifier les infomations de cette vente</h4>
      <hr />
      <form action="" method="POST">
        ...
        <div class="commande table-responsive">
          <table class="table table-bordered">
            ...
            <tbody>
              <tr
                v-for="(line, index) in form.sell_lines"
                :key="index"
                v-show="line._destroy != 1"
              >
                <td class="table-coll">
                  <v-select
...
export default {
  ...
  methods: {
    //recupérer les infos de la facture concernée
    recupFacture() {
      this.$axios
        ...
        .then((response) => {
          ...
          (this.form.date_sell = moment(vente.date_sell).format(
            "YYYY-MM-DDThh:mm"
          )),
    ...
    async submit() {
      ...
      await this.$axios
        ...
        .then((response) => {
          let vente = response.data.data[0];
          this.$axios
            .put("/sells/" + this.$route.params.id, {
    ...
    //modifier la quantite dans les lignes de vente
