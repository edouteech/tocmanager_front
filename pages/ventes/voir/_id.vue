<template >
  <div>
    ...
    <div class="alert alert-danger d-flex" role="alert" v-if="error">
      <div>{{ error }}</div>
      ...
      <table class="total d-flex align-items-end flex-column">
        <tbody>
          <tr>
            ...
            <td class="py-2 px-5">
              <strong>{{ montant_ht }} F CFA</strong>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td class="py-2 px-5">
              <strong>{{ montant_ttc }} F CFA</strong>
            </td>
          </tr>
          <tr>
    ...
    <div class="hidden_page">
      ...
      <div>
        <p>
          <strong> Total Hors Taxe : {{ montant_ht }} F CFA</strong>
        </p>
        <p>Taxe : {{ tax }}</p>
        <p>
          <strong> Total TTC : {{ montant_ttc }} F CFA</strong>
        </p>
        <p>Réduction: {{ discount }} F CFA</p>
        <p>
          <strong> Net à payer : {{ montant }} F CFA</strong>
        </p>
        <p>
...
export default {
  ...
  methods: {
    ...
    //détails de la facture concernée
    refresh() {
      this.$axios
    ...
    async submit() {
      if (this.form.montant > this.rest) {
        var that = this;
        this.error_rest =
          "Le montant à encaisser ne doit pas etre supérieur à la somme due";
        that.form.montant = "";
      } else {
        await this.$axios
          .post("/encaissements", {
          ...
          .then((response) => {
            if (response.data.status == "success") {
              ...
              that.form.montant = "";
              that.form.date = "";
              this.recupFacture(), this.recupInfos();
            } else {
    ...
    //valider la facture comme normalisé
    validerFactureNormalise() {
      this.$axios
        .get("/sells/valid/" + this.$route.params.id, {
          params: {
            compagnie_id: localStorage.getItem("auth.company_id"),
          },
        })
    ...
    genererFactureNormalise() {
      this.$axios
        .get("/sells/invoice/" + this.$route.params.id, {
          params: {
            compagnie_id: localStorage.getItem("auth.company_id"),
          },
        })
