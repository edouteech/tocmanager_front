...
export default {
  ...
  methods: {
    ...
    //ajout d'un produit par son code
    async codeAdd() {
      await this.$axios
        .get("/products/search", {
          params: {
    ...
    //rappel de refresh après le modal d'ajout de client
    setMessage(payload) {
      this.refresh();
    },

    //rappel de recupProduct après le modal d'ajout de produit
    setProduit(payload) {
      this.recupProduct();
    },

    //recherche des produits par code
    searchCode() {
      this.$axios
        .get("/products", {
          params: {
