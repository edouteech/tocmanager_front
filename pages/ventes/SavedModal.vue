    ...
    <div class="modaler" @click.stop>
      ...
      <form action="">
        <div class="form-group notify">
          <strong>Confirmer l'email auquel il faut envoyer la facture </strong
          >{{ email_client }}
          <div class="col-md-11 mx-auto row">
            <div class="col-md-8">
              <input
                type="email"
                class="form-control mx-auto mt-1"
                v-model="form.email"
                autocomplete="off"
                required
            ...
            <div class="col-md-4">
              <button class="btn btn-success mt-1" @click.prevent="submit()">
                Envoyer facture
              </button>
            </div>
          </div>
      ...
      <div class="mt-3">
        <NuxtLink :to="'/ventes/voir/' + this.identifiant"
          ><button class="btn btn-light">Voir la vente</button></NuxtLink
        >
        <button class="btn btn-warning mx-3" @click.prevent="newVente()">
          Nouvelle facture
        </button>
        <NuxtLink to="/dashboard"
          ><button class="btn btn-primary">Tableau de bord</button></NuxtLink
        >
      </div>
      <div class="mt-5 d-flex col-md-8 mx-auto">
        <button class="btn btn-dark mx-3" @click.prevent="generatePdf()">
          <i class="fa fa-print text-primary mx-1" aria-hidden="true"></i
          >Imprimer facture
        </button>
        <button class="btn btn-dark" @click.prevent="generateOtherpdf()">
          <i class="fa fa-print text-primary mx-1" aria-hidden="true"></i
          >Imprimer ticket
        </button>
      </div>
    ...
    <div class="imprim" id="impression">
      <div class="d-flex align-items-start flex-column">
        <div class="entreprise-photo mb-2" v-if="entreprise.logo">
          <img
            :src="$config.webURL + entreprise.logo"
            alt="profil"
            class="profil"
            width="70"
            height="50"
      ...
      <div class="d-flex align-items-end flex-column mb-4" v-if="facture_qr">
        <img
          :src="
        ...
        <div class="d-flex align-items-start flex-column mb-4">
          <div>
            <strong>Mecef_counteurs :</strong>{{ facture_qr.mecef_counteurs }}
          </div>
          <div><strong>Mecef_nim: </strong>{{ facture_qr.mecef_nim }}</div>
          <div><strong>Mecef_date :</strong> {{ facture_qr.mecef_date }}</div>
          <div>
            <strong>CodeMECeFDGI : </strong>{{ facture_qr.codeMECeFDGI }}
          </div>
        </div>
      </div>
      <table class="total d-flex align-items-end flex-column">
        <tbody>
          ...
          <tr>
            ...
            <td class="px-5">
              <strong class="text-warning">{{ rest_fact }} F CFA</strong>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...
    submit() {
      this.$axios
        .post("/sells/" + this.identifiant + "/notify", {
    ...
    //recupère les informations de la vente
    chargement() {
      (this.fact_id = this.recupFacture.id),
        (this.recFacts = this.recupFacture.sell_lines),
        (this.date_fact = moment(this.recupFacture.date_sell).format(
    ...
    //modal d'impression de ticket de caisse
    async generateOtherpdf() {
      this.showModal = true;
      (this.identifiant3 = this.recupFacture.sell_lines),
        (this.identifiant1 = moment(this.recupFacture.date_sell).format(
          "D MMM YYYY, h:mm:ss a"
        )),
